...
const CHANNELS = [
  ...
  {
    id: 'kukh_alomlat',
    label: 'شركة القصر للصرافة (@Kukh_alomlat)',
    url: 'https://t.me/s/Kukh_alomlat',
    // a Baghdad exchange (حي العدل). Its posts quote buy and sell 150 apart;
    // the first figure on the dollar line is taken, and averaging across the
    // Baghdad sources smooths any buy/sell difference
    cities: ['baghdad'],
    defaultCities: ['baghdad']
  },
  {
    id: 'iqborsa',
    label: 'بورصة العراق (@iqborsa)',
    url: 'https://t.me/s/iqborsa',
    cities: ['baghdad'],
    defaultCities: ['baghdad']
  },
  {
    id: 'nrxidolar',
    label: 'نرخی دۆلار (@nrxidolar)',
    url: 'https://t.me/s/nrxidolar',
    // Names the city next to the number ("بۆڕسەی هەولێر 154,000"), so
    // city-adjacency does the work. The fallback is still configured for the
    // posts that give one regional figure without naming a city.
    cities: ['erbil', 'sulaymaniyah', 'duhok'],
    defaultCities: ['erbil', 'sulaymaniyah', 'duhok']
  },
  {
    id: 'pmcgroup',
    label: 'PMC نرخی دۆلار (@PMCgroup)',
    url: 'https://t.me/s/PMCgroup',
    cities: ['erbil', 'sulaymaniyah', 'duhok'],
    defaultCities: ['erbil', 'sulaymaniyah', 'duhok']
  },
  {
    id: 'bazari_dolaraka',
    label: 'بازاڕی دۆلارەکە (@bazari_dolaraka)',
    url: 'https://t.me/s/bazari_dolaraka',
    // posts are just "$100=154,050" — no city, no dollar word
    cities: ['erbil', 'sulaymaniyah', 'duhok'],
    defaultCities: ['erbil', 'sulaymaniyah', 'duhok']
  },
  {
    id: 'pashagoldd',
    label: 'Pasha Gold - نرخی دراوەکان (@pashagoldd)',
    url: 'https://t.me/s/pashagoldd',
    // "#دۆلار بەرامبەر #دیناری_عێراقی" then "$100 = 154,150"
    cities: ['erbil', 'sulaymaniyah', 'duhok'],
    defaultCities: ['erbil', 'sulaymaniyah', 'duhok']
  },
  {
  ...
  ,
  {
    ...
    url: 'https://tekanexchange.net/api/rates',
    type: 'json',
    city: 'erbil',
    side: 'mid',
    // The response is a BARE ARRAY (no wrapper object), so listPath is empty.
    // Rates are already per $1 — buyRate 1560 / sellRate 1565 — not per $100.
    listPath: '',
    match: { field: 'currencyCode', value: 'USD' },
    buyField: 'buyRate',
    sellField: 'sellRate',
    // Each row carries its own `updatedAt`, and this endpoint has been seen
    // serving a USD figure months out of date. A stale rate published as live
    // is worse than no rate, so anything older than this is rejected and the
    // channel reports success:false with the age in its error.
    updatedAtField: 'updatedAt',
    maxAgeDays: 7,
    // OFF for now: when checked, their USD row was 66 days old while the
    // market had moved ~20 dinars. Leaving it on would fetch and fail on every
    // request and show a permanent "unavailable" source in the app, which is
    // misleading for something known-stale rather than temporarily down.
    //
    // Flip to true once their endpoint updates regularly — check the USD row's
    // `updatedAt` at https://tekanexchange.net/api/rates. Everything else is
    // already configured and tested; no other change is needed.
    enabled: false
  }
];

/** Channels actually in use. `enabled: false` keeps config without fetching. */
const ACTIVE_CHANNELS = CHANNELS.filter(c => c.enabled !== false);
...
const NOT_A_RATE_WORDS = [
  ...
  // these channels also post news, which is full of "155 billion dollars"
  // style figures; excluding the scale words keeps them out
  'ملیار', 'مليار', 'بلیۆن', 'بليون', 'trillion', 'billion',
  // and market/stock chatter that mentions dollars without being a rate
  'پشک', 'سهم', 'کۆمپانیا', 'شركة',
  // @iqborsa's boilerplate quotes the CBI's official 132,000 per $100. That
  // parses to a plausible 1320 and would be published as a MARKET rate, which
  // is exactly what the source picker exists to keep separate.
  'المركزي', 'مركزي', 'الرسمي', 'رسمي', 'ناوەندی', 'فەرمی'
];
// '$' matters as much as the words: several channels post nothing but
// "100$=154,100" or "$100 = 154,150", with no دۆلار anywhere on the line.
const DOLLAR_WORDS = ['دۆلار', 'دولار', 'dollar', 'usd', '$'];
const RATE_WORDS = ['نرخ', 'سعر', 'rate'];

/**
 * True when the text names a city the channel is NOT scoped to.
 *
 * Guards the labeled-rate fallback: if a Kurdistan-scoped channel posts
 * "بغداد 154.100", city-adjacency finds nothing (Baghdad is filtered out) and
 * the fallback would otherwise publish that Baghdad number as a Kurdistan
 * rate. Seeing an excluded city named at all means the post isn't a
 * region-wide rate, so the fallback stays out of it.
 */
function mentionsExcludedCity(rawText, allowedCities) {
  if (!allowedCities || !allowedCities.length) return false;
  const text = normalizeArabic(rawText);
  return CITIES
    .filter(c => !allowedCities.includes(c.id))
    .some(c => c.aliases.some(a => text.includes(normalizeArabic(a))));
}
...
function extractLabeledRate(rawText) {
  ...
  const numbersIn = (line) => (line.match(/\d[\d.,]{2,}/g) || [])
    .map(toRatePerUsd)
    .filter(Boolean);

  lines.forEach((line, i) => {
    if (!dollars.some(w => line.includes(w))) return;      // must mention dollars
    if (excluded.some(w => line.includes(w))) return;      // ...but not gold/ounce/official

    let nums = numbersIn(line);

    // Bourse-style posts put the label on one line and the figures on the
    // next ("سعر الدولار" / "بيع 154,250  شراء 154,100"), so fall through to
    // the following line when this one carries no usable number — but only if
    // that line isn't itself excluded.
    if (!nums.length) {
      const next = lines[i + 1];
      if (next && !excluded.some(w => next.includes(w))) nums = numbersIn(next);
    }
    if (!nums.length) return;

    // a line that also says "price/نرخ/سعر" is the strongest signal
...
async function scrapeJsonSource(channel) {
  ...
  // Reject data that is too old to be a live rate. Some endpoints keep serving
  // their last value indefinitely, with only `updatedAt` revealing that it
  // hasn't moved in months — publishing that as today's rate would be wrong.
  let ageDays = null;
  const stampField = channel.updatedAtField;
  if (stampField && row[stampField]) {
    const stamp = new Date(row[stampField]);
    if (!isNaN(stamp)) {
      ageDays = Math.floor((Date.now() - stamp.getTime()) / 86400000);
      const limit = channel.maxAgeDays || 7;
      if (ageDays > limit) {
        throw new Error(
          `${channel.id}: ${m.value} rate is ${ageDays} days old `
          + `(updated ${row[stampField]}), older than the ${limit}-day limit`
        );
      }
    }
  }
...
async function scrapeChannel(channel) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (!Object.keys(found).length && fallbackCities
        && !mentionsExcludedCity(text, channel.cities)) {
